...
- **T5** — visual QA scoped honestly: headless `VisualQaTests` (map load + model skeleton/bind-pose + shader-compile)
  in `ci.sh`; **windowed visual correctness needs a manual eye-check** (`tools/visual-qa.sh` + ../docs/RUNNING.md) — headless
  renders blank.
...
| **T62** | ◐ | **[demo] Demo recording — `.xgd` format + recorder (always-on, client+server).** `DemoFormat` (header/frame/index; reuse `BitWriter`/`EntityStateCodec`/`SoundWire`) + `DemoRecorder : IDemoSink` (keyframe cadence + file writer) + client-side recorder tap + finalize-at-boundary; wire `DemoControl.StartRecording/StopRecording`; flip `sv_autodemo`/`cl_autodemo` defaults **on** (deliberate, comment it). *Done when* matches auto-write `.xgd` on both sides + headless round-trip test passes. Spec `specs/demo-replay-and-spectator.md` §4–5,11–13. Ref `Base/darkplaces/cl_demo.c`. **STATUS (2026-07-08, merge f0d555e):** SERVER side DONE — `src/XonoticGodot.Net/Demo/{DemoFormat,DemoRecorder}.cs` (streaming, event packets, crash-recovery index, v2 roster trailer) wired via `NetGame.SetupDemoHooks`/`ServerNet.DemoSink`, `sv_autodemo 1` + `demo_keyframe_interval` live, headless round trip verified. **Remainder = T62b client tap** (`cl_autodemo` binds a cvar nothing reads) — do EARLY, right after the format-v3 batch (spec §15). | `src/XonoticGodot.Net/Demo/*`, `game/net/{NetGame,ServerNet}.cs`, `src/XonoticGodot.Server/DemoControl.cs`; T62b: `game/net/ClientNet.cs` | — |
| **T63** | ◐ | **[demo] Replay host + time control + spectator cameras.** `GameWorld.ReplayMode` (match logic inert, spectator move kept) + `DemoPlayback : IReplayEntitySource` (playhead/`SampleAt`/seek/event-windowing/loop-sound reconstruct) + `ServerNet.ReplaySource` + `NetGame.ConfigureReplay` + `--playdemo`; two-clock pause/slow/fast/smooth-rewind/scrub + `ReplayControlBar`; `SpectatorCamera` (FreeFly/Follow/Director) + per-player view-angle record. **Perspective rules:** server demo = lossless (pick start perspective + switch live while watching); client demo = SAME modes but PVS-limited (free-cam/follow/director/scripted all work, data may be incomplete — non-blocking warning, NOT a lock); under capture the perspective is fixed at start. *Done when* `--playdemo` flies/follows/directs and scrub/slow-mo/rewind work. Spec §3,6–9,11–13. Ref `Base/darkplaces/cl_demo.c`, `qcsrc/client/{main,view}.qc`. **STATUS (2026-07-08, merge f0d555e):** replay host + two-clock pause/slow/fast/seek/smooth-rewind + `ReplayControlBar` + FreeFly/chase-Follow `SpectatorCamera` + event re-broadcast DONE. **Remainder (order per spec §15):** format v3 batch (score blocks → replay scoreboard, per-player view angles, loop-sound index, DemoKind+recorder id, marker sidecar), seek transient-clear + loop-sound reconstruct, first-person Follow + cycling, Director cam, timeline markers, jump-to-keyframe, free-cursor scrub toggle. | new `src/XonoticGodot.Server/DemoPlayback.cs`, `game/client/SpectatorCamera.cs`, `game/hud/ReplayControlBar.cs`; touch `game/net/{ServerNet,NetGame}.cs`, `src/XonoticGodot.Server/GameWorld.cs`, `game/client/{FirstPersonView,ClientWorld}.cs` | **T62** |
...
> [`specs/demo-replay-and-spectator.md`](specs/demo-replay-and-spectator.md) +
> [`specs/video-capture.md`](specs/video-capture.md) +
> [`specs/cinematic-playback-scripts.md`](specs/cinematic-playback-scripts.md). **Decisions (2026-06-10):**
