...
answers: percentiles + 1%/0.1% lows, a hitch census split into primaries vs recovery
...
def parse_log(path: Path) -> dict:
    ...
    for ln in lines:
        ...
        if m:
            ...
            extra = max(0, n - 1)
            if cls.endswith("·recovery"):
                recovery += extra
            else:
                census[cls] = census.get(cls, 0) + extra
            events.append({"t": t, "cls": cls.replace("·recovery", ""), "ms": hi, "n": extra})
            continue
        ...
        if m:
            ...
            events.append({"t": t, "cls": cls.replace("·recovery", ""), "ms": v, "n": 1})
