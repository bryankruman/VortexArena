...
| `src/XonoticGodot.Server/Commands.cs` | **T38, T46, T47, T56, T60** | Minigame/chat/generic/admin verbs + the client-command-bus gate all touch the command table. |
...
per-mode round HUD. **A4 next** (T41 client feedback, T42 overtime, T46 chat, T52 Q3/QL compat, T23/T24/T27).
...
| **T5** | ☐ | **[J] Systematic windowed visual QA.** Verify lightmaps/materials/bone-poses/patches/billboards across several real maps+models (ScreenshotHook exists for one). → *A checklist of N maps + key models verified on-screen.* | `game/ScreenshotHook.cs`, assets (read-mostly) | (better after T13) |
...
| **T24** | ☐ | **[J] MDL/MD2/ZYM/PSK importers** (verify shipped content needs them first; else formally cut). | new `src/XonoticGodot.Formats/*`, `game/assets/AssetLoader.cs` | — |
...
| **T63** | ☐ | **[demo] Replay host + time control + spectator cameras.** `GameWorld.ReplayMode` (match logic inert, spectator move kept) + `DemoPlayback : IReplayEntitySource` (playhead/`SampleAt`/seek/event-windowing/loop-sound reconstruct) + `ServerNet.ReplaySource` + `NetGame.ConfigureReplay` + `--playdemo`; two-clock pause/slow/fast/smooth-rewind/scrub + `ReplayControlBar`; `SpectatorCamera` (FreeFly/Follow/Director) + per-player view-angle record. **Perspective rules:** client demo locks to the recorded player's 1st-person; server demo = pick start perspective + switch live while watching; under capture the perspective is fixed at start. *Done when* `--playdemo` flies/follows/directs and scrub/slow-mo/rewind work. Spec §3,6–9,11–13. Ref `Base/darkplaces/cl_demo.c`, `qcsrc/client/{main,view}.qc`. | new `src/XonoticGodot.Server/DemoPlayback.cs`, `game/client/SpectatorCamera.cs`, `game/hud/ReplayControlBar.cs`; touch `game/net/{ServerNet,NetGame}.cs`, `src/XonoticGodot.Server/GameWorld.cs`, `game/client/{FirstPersonView,ClientWorld}.cs` | **T62** |
| **T64** | ☐ | **[demo] Demos menu backend** (supersedes the demo slice of **T23**): enumerate `demos/*.xgd` via VFS → filtered list; **Play** → `ConfigureReplay`; **Record to video** → FPS/resolution/format dialog → relaunch capture. *Done when* you can browse, play, and export a demo entirely from the menu. Spec §10 + `specs/video-capture.md` §3,6. | `game/menu/dialogs/DialogMediaDemo.cs` | **T63**, **T65** |
...
### 🎬 Demo replay & video-capture track (T62–T65)
...
> [`specs/video-capture.md`](planning/specs/video-capture.md). **Decisions (2026-06-10):** always-on auto-record on
> **both** client & server (default on, one-cvar opt-out); the menu does **playback + video export**, not manual
> record; video capture rides **Godot Movie Maker (`MovieWriter`)** → AVI always, optional ffmpeg transcode to
> mp4/ogv. **Suggested order:** T62 → T63 → (T65 ∥ T64). T62/T65 can be prototyped against a live `--host` session
> before the replay host (T63) lands.
...
(T36·T43·T37·T51·T56·T28·T30) · **Wave A3** (T39·T57·T48·T53·T54·T26·T31·T33) — all landed + adversarially
reviewed, **build clean, 1446 tests pass**. See the "Recently landed" blocks above + the
[§ Wave-1 fidelity audit](#-wave-1-fidelity-audit-2026-06-06--vs-base-source).

### Forward plan A1–A6 (open tasks: T5, T23–T33, T35–T61, T62–T65, carried T11c/T19c) — re-audit 2026-06-07
...
| **A4** | **T41** client feedback (hitsound/footsteps/rings) · **T42** overtime/sudden-death · **T46** team/private chat · **T52** Q3/QL compat (needs T35) · **T23** menu data-lists · **T24** importers · **T27** HUD editor | *Faithful feel*: hitsound, OT, `say_team`, imported maps populated | NG/PP/HN(T41)·GW(T42)·CMD/SN(T46)·REG(T52)·menu(T23)·assets(T24) |
| **A5** | **T45** warpzone combat+render · **T49** campaign · **T47** harden cmd bus *(security)* · **T59** map long-tail · **T29** remaining HUD panels · **T61** announcer/notif polish · **T5** windowed visual QA | *Portals, campaign, security, polish*: combat through portals, SP works, clients can't run admin verbs | DMG/WZ/NG(T45)·GW(T49)·CMD/SN(T47)·REG(T59)·HN(T61) |
| **A6** | **T55** server fidelity (Porto, needs T45) · **T11c** nade-throw input · **T25** bot scripting+wpeditor · **T32** GameTypeVote+mapvote ballot · **T60** admin/ops breadth | *Command/admin tail*: Porto portals, throwable nades, bot_cmd, vote ballot UI | T25/T32/T60 share `Commands.cs`/`VoteController` → coordinate-additive, or **A0 → fully parallel** |
