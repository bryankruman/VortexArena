...
**A5 next** (T45 warpzone combat+render, T49 campaign, T47 cmd-bus security, T59 map long-tail, T29 HUD panels, T61 notif
polish, T5 visual QA, T68 shownames, T69 HUD shake).
...
| **T24** | ☑ | **[J] MDL/MD2/ZYM/PSK importers** → **FORMALLY CUT** (A4): only 2 MDL ship into the model loader (`casing_shell`/`chunk`) and both already fall back cleanly; the other 17 MDL never hit `LoadModel`, the 2 ZYM are map-pack addons, MD2/PSK ship zero content — ~2000 LOC for nil benefit. Documented in `AssetLoader.BuildModelFactory` + guarded by `ModelImporterCutTests`. | `game/loaders/AssetLoader.cs` (comment), `tests/*` | — |
...
(T41·T42·T46·T52·T23·T27·T66·T67 + T24 cut) — all landed + adversarially
reviewed, **build clean, 1735 tests pass**. See the "Recently landed" blocks above + the
...
| **A5** | **T45** warpzone combat+render · **T49** campaign · **T47** harden cmd bus *(security)* · **T59** map long-tail · **T29** remaining HUD panels · **T61** announcer/notif polish · **T5** windowed visual QA · **T68** shownames overlay · **T69** HUD dynamic shake | *Portals, campaign, security, polish*: combat through portals, SP works, clients can't run admin verbs, nametags + HUD shake | DMG/WZ/NG(T45)·GW(T49)·CMD/SN(T47)·REG(T59)·HN(T61)·client(T68/T69 — T69 after T41) |
